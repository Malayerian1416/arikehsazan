...
@section('page_title')
    <span class="iran_yekan external_page_title_text text-muted text-center">ایجاد عنوان اصلی منو</span>
@endsection
@section('content')
    <form id="create_form" action="{{route("MenuTitles.store")}}" method="post" data-type="create" v-on:submit="submit_form">
        ...
        <div class="form-row">
            ...
            <div class="form-group col-md-12 col-lg-3">
                ...
                <input type="text" class="form-control text-center @error('main_route') is-invalid @enderror ltr" id="main_route" name="main_route" value="{{old("main_route")}}">
                @error('main_route')
                <span class="invalid-feedback iran_yekan small_font" role="alert">{{ $message }}</span>
                @enderror
            </div>
