...
@section('scripts')
    <script>
        @if(isset($contract_categories) && isset($contract_category_id))
        let searches = @json($contract_categories);
        let related_data_select = {{$contract_category_id}};
        @endif
        @if(isset($contracts) && isset($contract_id))
        let searches_child = @json($contracts);
        let related_data_select_child = {{$contract_id}};
        @endif
        @if(isset($project_id))
            let parent_id = {{$project_id}};
        @endif
    </script>
@endsection
@section('page_title')
    <span class="iran_yekan external_page_title_text text-muted text-center">گزارشات پیمان</span>
@endsection
@section('content')
    <form action="{{route("Reports.contract_reports_make")}}" method="post">
        @csrf
        <div class="form-row border rounded pb-2">
            ...
            <div class="form-group col-md-12 col-lg-4 col-xl-3">
                ...
                <select class="form-control select_picker iran_yekan @error('project_id') is-invalid @enderror" title="انتخاب کنید" data-size="10" data-live-search="true" id="project_id" name="project_id" v-model="parent_id">
            ...
            <div class="form-group col-md-12 col-lg-4 col-xl-2">
                ...
                <select class="form-control select_picker iran_yekan @error('contract_branch_id') is-invalid @enderror" title="انتخاب کنید" data-size="10" data-live-search="true" id="contract_branch_id" name="contract_branch_id" v-on:change="related_data_search" data-type="contract_category">
            ...
            <div class="form-group col-md-12 col-lg-4 col-xl-2">
                <label class="col-form-label iran_yekan black_color" for="contract_category_id">
                    سرفصل پیمان
                    <i v-show="related_data_search_loading" class="button_loading fa fa-spinner fa-spin mr-2"></i>
                </label>
                <select class="form-control select_picker iran_yekan @error('contract_category_id') is-invalid @enderror" title="انتخاب کنید" data-size="10" data-live-search="true" id="contract_category_id" name="contract_category_id" v-model="related_data_select" v-on:change="related_data_search_child" data-type="contract">
            ...
            <div class="form-group col-md-12 col-lg-4 col-xl-2">
                <label class="col-form-label iran_yekan black_color" for="contract_id">
                    عنوان پیمان
                    <i v-show="related_data_search_loading_child" class="button_loading fa fa-spinner fa-spin mr-2"></i>
                </label>
                <select class="form-control select_picker iran_yekan @error('contract_id') is-invalid @enderror" title="انتخاب کنید" data-size="10" data-live-search="true" id="contract_id" name="contract_id" v-model="related_data_select_child">
                    <option v-for='search in searches_child' v-bind:value="search.id">@{{ search.name }}</option>
                </select>
                @error('contract_id')
                <span class="invalid-feedback iran_yekan small_font" role="alert">{{ $message }}</span>
                @enderror
            </div>
            <div class="form-group col-md-12 col-lg-3 col-xl-1">
            ...
            <div class="form-group col-md-12 col-lg-3 col-xl-1 align-self-end">
        ...
        <div class="table-responsive col-12">
            <table class="table table-striped iran_yekan index_table" id="main_table" data-filter='[1,2,3,4]'>
                ...
                @if(isset($results))
                    ...
                    @empty
                        <tr><td colspan="9"><span>اطلاعاتی یافت نشد</span></td></tr>
                    @endforelse
                    </tbody>
                @endif
            </table>
