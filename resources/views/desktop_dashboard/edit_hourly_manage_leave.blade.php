...
@section('content')
    <form id="update_form" action="{{route("Leaves.update",$leave->id)}}" method="post" data-type="update" v-on:submit="submit_form" enctype="multipart/form-data">
        @csrf
        @method('put')
        <input name="leave_type" type="hidden" value="hourly">
        <div class="form-row pb-2">
            ...
            <div class="form-group col-md-12 col-lg-3 col-xl-2">
                ...
                <select class="form-control iran_yekan select_picker @error('staff_id') is-invalid @enderror" title="انتخاب کنید" data-size="10" data-live-search="true" id="staff_id" name="staff_id">
                    @forelse($users as $user)
                        <option @if($leave->staff_id == $user->id) selected @endif value="{{$user->id}}">{{$user->name}}</option>
                    @empty
                    @endforelse
                </select>
                @error('staff_id')
            ...
            <div class="form-group col-md-12 col-lg-3 col-xl-2">
                ...
                <input type="text" id="location_name" readonly class="form-control iran_yekan" value="{{$leave->location ? $leave->location->name : "نامشخص"}}">
            </div>
            <div class="form-group col-md-12 col-lg-3 col-xl-2">
                <label class="col-form-label iran_yekan black_color" for="departure">
                    شروع مرخصی
                </label>
                <input type="time" name="departure" value="{{$leave->departure}}" class="form-control text-center iran_yekan @error('departure') is-invalid @enderror" style="height: 30px">
                @error('departure')
                <span class="invalid-feedback iran_yekan small_font" role="alert">{{ $message }}</span>
                @enderror
            </div>
            <div class="form-group col-md-12 col-lg-3 col-xl-2">
                ...
                <input type="time" name="arrival" value="{{$leave->arrival ?: ""}}" class="form-control text-center iran_yekan @error('arrival') is-invalid @enderror" style="height: 30px">
                @error('arrival')
                <span class="invalid-feedback iran_yekan small_font" role="alert">{{ $message }}</span>
                @enderror
            </div>
            <div class="form-group col-md-12 col-lg-3 col-xl-2">
                ...
                <input type="file" hidden class="form-control iran_yekan text-center @error('leave_docs') is-invalid iran_yekan @enderror" v-on:change="file_browser_change" multiple id="agreement_sample" name="leave_docs[]" accept=".pdf,.doc,.docx,.jpg,.png,.bmp,.jpeg,.xls,.xlsx,.txt">
...
@section('page_footer')
    <div class="form-row m-0 d-flex flex-row justify-content-center">
        ...
        <a href="{{route("idle")}}" class="btn btn-outline-light iran_yekan">
