...
@section('scripts')
    <script>
        ...
        let parent_id = {{$contractor_id}};
        @endif
        @if(isset($projects) && isset($project_id))
        let searches = @json($projects);
        let related_data_select = {{$project_id}};
        @endif
        @if(isset($projects) && isset($project_id))
...
@endsection
@section('content')
    <form action="{{route("Reports.contractor_reports_make")}}" method="post">
        @csrf
        <div class="form-row border rounded pb-2">
            ...
            <div class="form-group col-md-12 col-lg-4 col-xl-2">
                <label class="col-form-label iran_yekan black_color" for="contractor_id">
                    پیمانکار
                </label>
                <select class="form-control select_picker iran_yekan @error('contractor_id') is-invalid @enderror" title="انتخاب کنید" data-size="10" data-live-search="true" id="contractor_id" name="contractor_id" v-on:change="related_data_search" data-type="contractor_project" v-model="parent_id">
                    @forelse($contractors as $contractor)
                        <option @if($contractor->id == old("contractor_id")) selected @elseif(isset($contractor_id) && $contractor->id == $contractor_id) selected @endif value="{{$contractor->id}}">{{$contractor->name}}</option>
                    @empty
                    @endforelse
                </select>
                @error('contractor_id')
            ...
            <div class="form-group col-md-12 col-lg-4 col-xl-3">
                ...
                <select class="form-control select_picker iran_yekan @error('project_id') is-invalid @enderror" title="انتخاب کنید" data-size="10" data-live-search="true" id="project_id" name="project_id" v-model="related_data_select" v-on:change="related_data_search_child" data-type="contractor_project_contract">
                ...
                @if(isset($results))
                    ...
                    @forelse($results as $result)
                        <tr v-on:click="invoice_details" data-invoice_id="{{$result->id}}">
                            ...
                            <td><span>{{$result->contract->project->name}}</span></td>
                            <td><span>{{$result->contract->category->branch->branch}}</span></td>
